...
def build_b01(sh):
    ...
    row("CS_rate", "G10: Core Scientific-AMD powered shell (derived)", f"={R('CS_tot')}/{R('CS_MW')}/{R('CS_term')}", "$M per MW-yr", "L-148", "confirmed", "T1 (derived)", "2026-07-28", "equals 14,000 / 530 / 15", "STABLE", "00, 09", nf=CNT2)
    row("BP_rate", "G10: Hut 8 Beacon Point powered shell (derived)", f"={R('BP_val')}/{R('BP_MW')}/{R('BP_term')}", "$M per MW-yr", "L-167", "confirmed", "T1 (derived)", "2026-07-20", "equals 19,600 / 704 / 15 (~155 per kW-month)", "STABLE", "00, 09", nf=CNT2)
    row("RIOT_rate", "G10: Riot powered shell (derived)", f"={R('RIOT_tot')}/{R('RIOT_MW')}/{R('RIOT_term')}", "$M per MW-yr", "L-136", "confirmed", "T2 (derived)", "2026-08-10", "equals 9,100 / 191 / 20", "STABLE", "00, 09", nf=CNT2)
    ...
    row("FS_NSCALE", "G11: full-stack lease rate, Nscale (derived)", f"={R('NSCALE_tot')}/{R('NSCALE_MW')}/{R('NSCALE_term')}", "$M per MW-yr", "L-134", "estimated", "T3 (derived)", "2026-08", "equals 45,000 / 460 / 6", "VOLATILE", "00, 09", nf=CNT2)
    row("FS_VOLTA", "G11: full-stack lease rate, Volta (derived)", f"={R('VOLTA_tot')}/{R('VOLTA_MW')}/{R('VOLTA_term')}", "$M per MW-yr", "L-134", "estimated", "T3 (derived)", "2026-08", "equals 10,000 / 133 / 6", "VOLATILE", "00, 09", nf=CNT2)
    ...
    row("INF_SHARE25", "G18: OpenAI inference as % of revenue, 2025 (derived; the Y1 anchor of the % path)", f"={R('OAI_INF25')}/{R('OAI_REV25')}", "fraction", "L-083, L-063", "estimated", "T3 (derived)", "2025", "equals 8,400 / 13,100", "QUARTERLY", "00", nf=PCT)
...
def build_b02(sh):
    ...
    for s in (1, 2, 3):
        ...
        line(sh, r + 4, "CMP_CAPEX", s, "Purchased chips ($M)", "$M", "equals purchases x price / 1e6", {y: f"={YB[y]}{r+2}*{YB[y]}{r+3}/{R('MPERUSD')}" for y in YRS}, bold=True)
