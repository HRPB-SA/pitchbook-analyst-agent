...
Sources: Unicorn 20 valuations+pricing (model marks daily, to 7/15/2026);
...
Three marks per top-20 name: MODEL (Unicorn 20 daily, 7/15), ROUND (Monitor
last-round post-money ~6/22), SECONDARY (Monitor implied = price x shares).
...
with a round mark, MODEL totals $2,800B vs ROUND $2,550B = **model sits +9.8%
...
- Model is most bullish; secondary corroborates the direction but not the
  magnitude - the model marks the top names ~10-14% richer than where shares
  actually trade.
...
- Neuralink is a re-rate outlier: model $41.9B / secondary $42.8B vs a $9.7B 2021
  round (+332%) - the round is ancient, not the company cheap.

**Value creation / YTD 2026 (model marks).** The 20 sum to $4,453B. YTD spread is
enormous: Perplexity +755%, Cerebras +482%, Anthropic +219%, Neuralink +205%,
SpaceX +93%, Ramp +77%, OpenAI +71% - against outright decliners Ripple -27%,
Rippling -15%, Canva -13%, Kraken -13%, Deel -9%. Value creation is concentrated:
Anthropic (inception index $4.0B -> $1,120.7B) and SpaceX ($75B -> $1,605B) alone
added over $2.6T of model value; mature names (Stripe $121B -> $169B, Applied
...
Anthropic alone = ~48% of AI value on model marks (44% on round marks) - "AI
...
$1,606B, Anthropic $1,121B, OpenAI $872B, Stripe $169B, Databricks $157B,
...
high mark reliability; names where they diverge widely (Neuralink +332%, Ripple
-53%, Ramp +40% model/round with secondary far off, Kraken) have low mark
reliability - a governance/valuation-quality flag. The megacaps sit in between:
model bullish, secondary 10-14% lower.
