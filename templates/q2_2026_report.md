...
The figure to track is not the aggregate, which will keep climbing on a dozen AI valuations, but the share of value resting on the frontier of recently tested valuations against the interior of dated ones. The event that will move it is the next set of frontier financings. SpaceX has already met its test, and public shares value the company roughly a third below its model valuation. Anthropic and OpenAI have not. Should Anthropic's expected listing or OpenAI's next round clear below the model rather than above it, the repricing will not stay contained to a single company.
