...
rows=[]
for name,s,cls,vp,vs,fl in D:
    rp=s-(a+b*np.log10(vp)); rs=s-(a+b*np.log10(vs))
    rows.append((name,s,cls,vp,vs,fl,rp,rs))
...
ax.text(0,20.55,"noise band (±0.5)",ha="center",va="bottom",fontsize=7.8,color=MUT,zorder=1)

n=len(rows); ys=list(range(n,0,-1))
for (name,s,cls,vp,vs,fl,rp,rs),yy in zip(rows,ys):
    ...
    ax.text(max(rp,rs)+0.13,yy,f"{rs:+.1f}{dag}{note}",va="center",ha="left",fontsize=8.3,color=SEC,zorder=5)
...
fig.text(0.008,0.944,f"Composite minus the score predicted by valuation (fit of score on log valuation, R² = {r2:.0%}), "
         "under primary and secondary pricing.",fontsize=9.8,color=SEC,va="top")
fig.text(0.008,0.922,"Size drops out by construction. The negative tail is entirely AI and frontier-science names.",
         fontsize=9.8,color=SEC,va="top")

leg=[Line2D([0],[0],marker="o",color="none",markerfacecolor=SURF,markeredgecolor=PRIM,
            markeredgewidth=1.8,markersize=9,label="Residual at the primary valuation († = model valuation)"),
...
fig.text(0.008,0.040,"Source: AIBQ/PBQ Panel as of 2026-06-30 (composites); Unicorn Monitor Top20_Marks (valuations). "
         "Label shows the residual at the secondary valuation. Residuals inside ±0.5 are within the noise band.",
...
with open(out,"w",newline="") as f:
    ...
    for line in [
     ...
     "# METHODOLOGY:",
     f"# 1. Fit an ordinary least squares line of composite score on log10(primary valuation $B) across the 20 companies: score = {a:.3f} + {b:.3f} * log10(V). R-squared = {r2:.3f}.",
     "# 2. residual_primary = composite - predicted score at the primary valuation (last-round post-money; Morningstar model valuation for SpaceX and Cerebras, which listed during the quarter and have no private last round - flagged 'model' in primary_basis).",
     "# 3. residual_secondary = composite - predicted score at the secondary valuation (secondary-implied value from observed share transactions; public trading value for SpaceX and Cerebras), using the SAME fitted line, so the difference between the two residuals reflects only the valuation gap between markets.",
     ...
     "# SOURCES: AIBQ/PBQ Panel as of 2026-06-30 (composites, point-in-time capped); Unicorn Monitor Top20_Marks (primary and secondary valuations).",
    ...
    for name,s,cls,vp,vs,fl,rp,rs in rows:
        pb="model_valuation" if fl=="m" else "last_round_post_money"
        sb="public_trading" if fl=="m" else "secondary_implied"
        w.writerow([name,cls,f"{s:.2f}",f"{vp:.2f}",pb,f"{vs:.2f}",sb,
                    f"{s-rp:.3f}",f"{rp:.3f}",f"{s-rs:.3f}",f"{rs:.3f}",
                    "above" if vs>=vp else "below","yes" if abs(rp)<0.5 else "no"])
print("wrote",out)
