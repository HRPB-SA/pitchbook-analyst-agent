"""House chart factory. Data-driven: a chart spec (JSON) in, a 300-DPI PNG out.

Two looks, selected per spec with "look":
    "house"  navy/slate/green institutional palette (deep-dive reports)
    "pb"     published analyst-note style: teal bars, near-black titles, grey
             unit subtitle, sources footer line ("Sources: ... | Geography: US
             | As of July 17, 2026"), value labels inside the bar start,
             optional shaded zone bands with right-edge labels
Spec fields "sources", "geography", "as_of" render the footer line (any look).
...
Spec shape: {"name": "out.png", "type": <type>, "look": "house"|"pb",
             "title": ..., "subtitle": ..., ...type-specific data...}.
See CHART_TYPES at bottom.
"""
...
PALETTES = {
    "house": {
        "primary": "#1F2A44", "alt": "#35506E", "accent": "#1C5D46",
        "muted": "#6B7280", "warn": "#8F3421", "soft": "#F2F5F9",
        "ink": "#2B3446", "title": "#1F2A44", "sub": "#6B7280",
        "band": "#EDF2F7", "edge": "#D5DBE4",
        "series": ["#1F2A44", "#35506E", "#1C5D46", "#8F3421", "#6B7280", "#5B7B9A"],
        "label_inside": False,
    },
    "pb": {
        "primary": "#2BC0C2", "alt": "#1F2A44", "accent": "#0E8385",
        "muted": "#605E5C", "warn": "#B0472E", "soft": "#EDF4F4",
        "ink": "#1A1A1A", "title": "#1A1A1A", "sub": "#605E5C",
        "band": "#E4F6F6", "edge": "#D9DEE3",
        "series": ["#2BC0C2", "#1F2A44", "#8AD9DA", "#0E8385", "#605E5C", "#C7CDD4"],
        "label_inside": True,
    },
}
...
def _pal(spec):
    return PALETTES[spec.get("look", "house")]


def _style_ax(ax, P, grid_axis="x"):
    for s in ("top", "right"):
        ax.spines[s].set_visible(False)
    for s in ("left", "bottom"):
        ax.spines[s].set_color(P["edge"])
    if grid_axis:
        ax.grid(axis=grid_axis, color=P["soft"], linewidth=0.9, zorder=0)
...
def _title(fig, spec, x=0.01):
    P = _pal(spec)
    fig.text(x, 0.985, spec["title"], ha="left", va="top", fontsize=10.8,
             fontweight="bold", color=P["title"])
    if spec.get("subtitle"):
        fig.text(x, 0.925, spec["subtitle"], ha="left", va="top", fontsize=7.6,
                 color=P["sub"])


def _footer(fig, spec):
    """Published-note sources line: Sources: ... | Geography: ... | As of ..."""
    parts = []
    if spec.get("sources"):
        src = spec["sources"]
        parts.append("Sources: " + (", ".join(src) if isinstance(src, list) else str(src)))
    if spec.get("geography"):
        parts.append("Geography: " + spec["geography"])
    if spec.get("as_of"):
        parts.append("As of " + spec["as_of"])
    if parts:
        fig.text(0.99, 0.005, "  •  ".join(parts), ha="right", va="bottom",
                 fontsize=6.2, color=_pal(spec)["muted"])


def _save(fig, spec, outdir):
    _footer(fig, spec)
    os.makedirs(outdir, exist_ok=True)
    path = os.path.join(outdir, spec["name"])
...
def _guard(spec):
    ...
    if "scatter" in t:
        raise EmbargoError(
            ...
            "coefficient is embargoed; use ranked_bars (per-point ladder).")
...
def ranked_bars(spec, outdir):
    """rows: [{label, value, accent?|muted?|alt?|est?, value_label?}], xlabel,
    xlim?, annotate {x,y,text,ha}?, bands [{rows:[i0,i1], label}] (pb zones)."""
    P = _pal(spec)
    rows = spec["rows"]
    ...
    xmax = spec.get("xlim") or max(vals) * 1.22
    for band in spec.get("bands", []):
        i0, i1 = band["rows"]
        ax.axhspan(i0 - 0.42, i1 + 0.42, color=P["band"], zorder=0)
        if band.get("label"):
            ax.text(xmax * 0.985, (i0 + i1) / 2, band["label"], va="center",
                    ha="right", fontsize=7.4, color=P["muted"])
    for i, r in enumerate(rows):
        col = P["accent"] if r.get("accent") else P["muted"] if r.get("muted") \
            else P["alt"] if r.get("alt") else P["primary"]
        est = bool(r.get("est"))
        ...
        if P["label_inside"] and not est:
            ax.text(max(vals) * 0.012, i, lbl, va="center", ha="left",
                    fontsize=8.2, fontweight="bold", color=P["ink"], zorder=4)
        else:
            ax.text(r["value"] + max(vals) * 0.015, i, lbl, va="center",
                    ha="left", fontsize=8.4, fontweight="bold",
                    color=col if r.get("accent") else P["ink"])
    ...
    if spec.get("annotate"):
        ...
        ax.text(a.get("x", max(vals)), a.get("y", len(rows) - 0.4), a["text"],
                fontsize=7.3, color=P["muted"], ha=a.get("ha", "right"),
                style="italic")
    _title(fig, spec)
    return _save(fig, spec, outdir)


def combo_bars_line(spec, outdir):
    """labels[], bars[], line[]; bar/line ylabels; accent_last?; line_ylim?"""
    P = _pal(spec)
    ...
    _style_ax(ax, P, "y")
    if line:
        lc = P["warn"] if spec.get("look", "house") == "house" else P["alt"]
        ax2 = ax.twinx()
        ax2.plot(x, line, color=lc, linewidth=2.0, marker="o", markersize=5.5, zorder=5)
        for i, v in enumerate(line):
            ax2.annotate(spec.get("line_fmt", "{:+g}%").format(v), (i, v),
                         textcoords="offset points", xytext=(0, 9), ha="center",
                         fontsize=8.2, fontweight="bold", color=lc)
        lo, hi = min(line), max(line)
        span = (hi - lo) or 1
        y0, y1 = spec.get("line_ylim", (lo - span * 0.7, hi + span * 0.35))
        ax2.set_ylim(y0, y1)
        ax2.set_ylabel(spec.get("line_ylabel", ""), fontsize=7.8, color=lc)
        ax2.tick_params(colors=lc, labelsize=7.5)
        ax2.spines["top"].set_visible(False)
        ax2.spines["right"].set_color(P["edge"])
    _title(fig, spec)
    return _save(fig, spec, outdir)
...
def series_bars(spec, outdir):
    ...
    cols = [P["accent"] if i in accents else P["primary"] for i in range(len(values))]
...
def football_field(spec, outdir):
    """rows: [{label, lo, hi, marker?, accent?}], vline {x, label}?, xlabel"""
    P = _pal(spec)
    rows = spec["rows"]
    ...
    for i, r in enumerate(rows):
        ax.barh(i, r["hi"] - r["lo"], left=r["lo"], height=0.5,
                color=P["accent"] if r.get("accent") else P["primary"],
                alpha=0.9, zorder=3)
        ax.text(r["hi"] + 2, i, r.get("range_label", f"{r['lo']:g}-{r['hi']:g}"),
                va="center", fontsize=7.8, color=P["ink"])
        if r.get("marker") is not None:
            ax.plot([r["marker"]], [i], marker="D", color=P["warn"],
                    markersize=6, zorder=5)
    if spec.get("vline"):
        v = spec["vline"]
        ax.axvline(v["x"], color=P["warn"], linewidth=1.2, linestyle="--", zorder=4)
        ax.text(v["x"], len(rows) - 0.25, " " + v["label"], fontsize=7.2,
                color=P["warn"], ha="left")
...
def radar(spec, outdir):
    ...
    for i, s in enumerate(spec["series"]):
        ...
        col = P["accent"] if s.get("accent") else P["series"][i % len(P["series"])]
    ...
    ax.set_yticklabels(["2", "4", "6", "8", "10"], fontsize=6.5, color=P["muted"])
    ax.grid(color=P["edge"], linewidth=0.6)
    ax.legend([s["label"] for s in spec["series"]], loc="lower right",
              bbox_to_anchor=(1.18, -0.08), fontsize=7.2, frameon=False)
    _title(fig, spec, x=0.02)
    return _save(fig, spec, outdir)
...
def gauge(spec, outdir):
    ...
    ax.plot(np.cos(th), np.sin(th), color=P["edge"], linewidth=14,
            solid_capstyle="butt")
    fr = (floor - lo) / (hi - lo)
    ...
    ax.annotate("", xy=(0.82 * np.cos(ang), 0.82 * np.sin(ang)), xytext=(0, 0),
                arrowprops=dict(arrowstyle="-|>", color=P["alt"], linewidth=2.4))
    fa = np.pi * (1 - fr)
    ax.plot([0.86 * np.cos(fa), 1.10 * np.cos(fa)],
            [0.86 * np.sin(fa), 1.10 * np.sin(fa)], color=P["warn"], linewidth=1.6)
    ax.text(1.13 * np.cos(fa), 1.13 * np.sin(fa), spec.get("floor_label", f"{floor:g}"),
            fontsize=7.6, color=P["warn"], ha="left" if fa < np.pi / 2 else "right")
    ax.text(0, -0.14, spec.get("value_label", f"{v:g}"), fontsize=17,
            fontweight="bold", color=P["title"], ha="center")
    ax.text(0, -0.30, spec.get("label", ""), fontsize=7.8, color=P["muted"],
            ha="center")
    ax.text(-1.02, -0.06, f"{lo:g}", fontsize=7, color=P["muted"], ha="center")
    ax.text(1.02, -0.06, f"{hi:g}", fontsize=7, color=P["muted"], ha="center")
...
def heatmap(spec, outdir):
    ...
    cmap = LinearSegmentedColormap.from_list("hmap", ["#FFFFFF", P["soft"], P["accent"]])
    ...
    for r in range(grid.shape[0]):
        for c in range(grid.shape[1]):
            ...
            ax.text(c, r, spec.get("fmt", "{:g}").format(grid[r, c]), ha="center",
                    ...
                    color="white" if frac > 0.62 else P["ink"])
            if hl:
                ax.add_patch(plt.Rectangle((c - .5, r - .5), 1, 1, fill=False,
                                           edgecolor=P["warn"], linewidth=2))
...
def composition(spec, outdir):
    ...
    ax.text(total * 1.01, 0, spec.get("total_label", f"{total:g}"), va="center",
            fontsize=8.4, fontweight="bold", color=P["ink"])
